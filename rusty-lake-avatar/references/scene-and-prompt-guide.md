...
For every selected named supporting character or animal, obtain a character-specific verified visual reference and label it separately in the image prompt. Do not let a generic figure or animal already present in the scene screenshot stand in for the selection. For Harvey, always supply `../assets/harvey-reference.png` and preserve its green parrot silhouette, olive head and torso, dark-green wing and long tail, white eye ring, and large hooked charcoal beak. In particular, the small brown bird in `The White Door` window scene is not Harvey.
...
- the fixed skin-plane base color shown in [../assets/face-color-reference.png](../assets/face-color-reference.png): median RGB `219, 197, 175` (`#DBC5AF`), applied to every visible human face, ear, neck, chest, arm, and hand, with almost no modeled volume and chosen independently of the source portrait and scene; write the exact RGB and hex value in the prompt and do not replace it with a semantic color approximation;
...
Derive environment color and lighting from the chosen screenshot instead of applying one universal palette to every game, but keep every unshadowed human face plane fixed to `#DBC5AF` even when the portrait or environment has a different color cast. Preserve ethnicity through proportions, facial geometry, and hair rather than realistic skin color. Explicitly reject color grading over the face, realistic skin texture, pores, subsurface glow, glossy photographic eyes, large iris/pupil rendering, detailed eyelashes, saturated lips, smooth digital-painting gradients, cinematic depth of field, and portrait-photography lighting.
...
Input images: Image 1 is the loose identity/edit target; Image <face-color reference> is the mandatory face-color target with base RGB 219, 197, 175 / #DBC5AF; later images are separately labeled verified references for scene, each named cast member, and wardrobe
...
Style/medium: hard-flat 2D Rusty Lake character art, irregular black contours, minimally shaded face whose unshadowed base plane is exactly RGB 219, 197, 175 / #DBC5AF from Image <face-color reference>; do not semantically reinterpret or scene-grade this color; default to large white eyes with small round pupils about 1.5–2 times the classic tiny-dot diameter and still clearly smaller than the eye whites; use bean/dot eyes only if explicitly requested; narrow schematic nose; pale near-skin thin-line mouth; solid hair locks with visible black interior strand lines; rigid illustrated anatomy; lightly aged texture
...
Constraints: user remains fully human; every unshadowed human face plane matches #DBC5AF and must pass the numeric validator; each selected named character matches its own verified reference; if Harvey is selected, include the bundled Harvey reference and render the green parrot, never the generic brown window bird; eyes follow one approved construction; hair has black internal linework; mouth has no saturated color; canonical background remains one location and no more line-dense than its verified screenshot; include every user-selected item and at least five separately visible canonical cues; no direct screenshot collage
...
- Skin-color drift: set every visible unshadowed human skin plane to RGB `219, 197, 175` (`#DBC5AF`) from `assets/face-color-reference.png`, remove all scene-color spill, keep every other visual unchanged, and rerun `scripts/validate_face_color.py`. If the single corrective image edit still misses only this color invariant, use `scripts/normalize_skin_color.py` once with explicit skin-region boxes, inspect that non-skin content is unchanged, and rerun validation.
- Wrong Harvey: replace only the substitute bird with the green parrot from `assets/harvey-reference.png`, preserving the olive head and torso, darker green wings and long tail, white eye ring, and hooked charcoal beak; keep the scene, subject, and composition unchanged.
